...
### 3.2 改用 Zephyr 原生 ssd1680 驅動 → 畫面雜訊
...
- 正確原因:通用驅動把整個 framebuffer 用**單次 `mipi_dbi_command_write`** 送出
  ...
  與 §3.3 看到的「單次大傳輸中間掉一段」**是同一個 LPSPI bug**。
  幾何那邊頂多少 2 行(122→120),不會整片雜訊;width/height 其實是可設的 property。

**(當下)結論**:先放棄通用驅動,改走忠實移植 user-space 序列(§3.3)。
正確的根因與「原生驅動其實可救」見 §3.4。

### 3.3 raw SPI 忠實移植 → 中間出現反相塊
...
### 3.4 根因更正:原生驅動失敗 = 同一個 LPSPI 大傳輸 bug(非幾何)

> **注意：機制已再次修正(以 §4 為準)**:本節下方把主因歸為「FIFO underrun / CS refill 邊界」、
> 並提出「降頻」「eDMA 根治」—— 這些**後來都被面板實測推翻**。實測:連 16-byte 單次填滿
> FIFO(零補料、零 underrun)都已壞;分界是**固定第 11 個 word**、與時脈無關、與 CONT/CONTC/
> watermark/RX 排空皆無關(見 §4)。underrun/降頻/eDMA 段保留作排查紀錄,**結論不採用**。

事後追原生驅動的寫 RAM 路徑,確認 §3.2 的「幾何不符」歸因**不正確**:
...
#### 3.4.2 原生驅動重試:用 property 降頻(sibling sample)
...
#### 3.4.4 根治:LPSPI3 改走 eDMA2(原生驅動可乾淨運作)
...
### 3.6 交叉驗證與治本嘗試(Linux 對照 + driver-fix)

§4 把根因定位到「LPSPI CPU/PIO 模式單筆多-word 傳輸掉資料」。後續做了兩件事把它釘死,並嘗試治本。

#### 3.6.1 Linux 同硬體交叉驗證(決定性)
...
## 4. Root Cause 分析（最終結論 RESOLVED）

> 以下是最後定論。§3 是完整排查歷程(含數個事後被推翻的中途假設),保留作紀錄。
...
| `samples/boards/nxp/frdm_imx93/epaper_pixpaper_ssd16xx/` | 原生驅動重試範例（§3.4,降頻實驗):`mipi-dbi-spi` + `solomon,ssd1680`,`mipi-max-frequency=1MHz` |
...
### 5.6 部署到 A55(Linux)與 remoteproc 啟動
...
> 前置條件提醒:
> - kernel 必須是含 5 處 `CLK_IS_CRITICAL`(§3.1)的版本,否則 `start` 後系統會卡死。
> - 不可套用 user-space spidev 測試用的那份 DT(§5.2),否則 LPSPI3/GPIO 會與 M33 相撞。
...
2. **大型 SPI 傳輸的隱性問題(真正的主因)**:在此平台,LPSPI CPU 模式下「單筆含 ≥2 個 word 的
   ...
   > 註:早期曾歸因 underrun 並嘗試降頻 / watermark / eDMA「根治」,**均被實測推翻**(§3.4 橫幅)。
